...
> - **status** (text, read-only) — embed สถานะ/วัน/ฤดู/ผู้เล่น + รายชื่อม็อด (ชื่อเป็นลิงก์ workshop)
...
| `/mods` | ม็อดที่เปิดใช้ (อ่าน `modoverrides.lua` แปลงเลข workshop → ชื่อผ่าน Steam) | |
...
src/
  ...
  dst/
    ...
    mods.ts                อ่าน modoverrides.lua + แปลงเลข workshop → ชื่อ (Steam API + cache)
  discord/
...
- **Mods** — อ่านจาก `modoverrides.lua` (per-shard; ลอง Master ก่อนแล้ว fallback shard อื่น)
  ดักกรณีไฟล์ไม่มี/พังไว้แล้ว แปลงเลข workshop → ชื่อผ่าน Steam Web API (public ไม่ต้องใช้ key)
  แล้ว cache ลง `mods-cache.json` 7 วัน (ปรับ `CACHE_TTL_MS` ใน `mods.ts` ได้); resolve ไม่ได้/
  ออฟไลน์ → fallback เป็นเลข + ลิงก์ workshop แสดงผลที่ `/mods`, status embed และหน้าเว็บ
...
ดูสถานะ shard/ผู้เล่น/วัน-ฤดู/ม็อด, แก้ `cluster.ini`, และปุ่มสั่งงาน DST
